...
export type SmartPostRepoOptions = {
  strategy: ReadStrategy;
};

export class SmartPostRepository implements PostRepository {
  constructor(
    private readonly pg: BasePostRepository,
    private readonly mongo: BasePostRepository,
    private readonly metrics: MetricsRecorder,
    private readonly decision: DecisionEngine,
    private readonly options: SmartPostRepoOptions,
  ) {}
  ...
  async create(input: CreatePostInput): Promise<Post> {
    ...
    return this.primaryMirrorWrite("create", () => this.pg.insert(post), () => this.mongo.insert(post), post);
  }
  ...
  async update(id: string, input: UpdatePostInput): Promise<Post | null> {
    ...
    return this.primaryMirrorWrite(
      "update",
      () => this.pg.update(id, payload),
      () => this.mongo.update(id, payload),
      null,
    );
  }
  ...
  private async primaryMirrorWrite<T>(
    ...
    void Promise.all([pgPromise, mgPromise]).then(([pg, mg]) => {
      ...
      if (!pg.ok) {
        console.warn(`[smart-repo] postgres ${op} failed: ${pg.error}`);
      }
      if (!mg.ok) {
        console.warn(`[smart-repo] mongo ${op} failed: ${mg.error}`);
      }
    });

    const primaryRes = await (primary === "postgres" ? pgPromise : mgPromise);
    if (primaryRes.ok) {
      return (primaryRes.value as T) ?? fallback;
    }

    const mirrorRes = await (primary === "postgres" ? mgPromise : pgPromise);
    if (mirrorRes.ok) {
      return (mirrorRes.value as T) ?? fallback;
    }
    throw new Error(
    ...
    try {
      const first = await Promise.any(
        tagged.map(async (p) => {
          const x = await p;
          if (!x.r.ok) {
            throw new Error(x.r.error ?? "read failed");
          }
          return x;
        }),
      );
